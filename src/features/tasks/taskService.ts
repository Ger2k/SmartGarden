...
export async function listTasks(userId: string): Promise<Task[]> {
  ...
  return snapshot.docs.map((item) => {
    ...
    return {
      ...
      weatherGuidance: data.weatherGuidance as Task['weatherGuidance'],
      weatherReason: typeof data.weatherReason === 'string' ? data.weatherReason : undefined,
...
export async function createTask(task: Omit<Task, 'id'>): Promise<void> {
  ...
  await addDoc(ref, {
    ...
    weatherGuidance: parsed.data.weatherGuidance ?? 'none',
    weatherReason: parsed.data.weatherReason ?? null,
